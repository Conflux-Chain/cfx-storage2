use cfx_storage2::{
    ...
    lvmt::{AmtNodes, AuthChangeTable, FlatKeyValue, SlotAllocations},
    middlewares::{
        table_schema::{HistoryChangeTable, HistoryIndicesTable},
        CommitIDSchema, HistoryNumberSchema,
    },
};
...
fn count<T: TableSchema>(view: Box<dyn '_ + TableRead<T>>) {
    ...
    println!(
        "- 键的总大小: {} 字节 (约 {:.2} MB)",
        total_key_size,
        total_key_size as f64 / 1_048_576.0
    );
    println!(
        "- 值的总大小: {} 字节 (约 {:.2} MB)",
        total_value_size,
        total_value_size as f64 / 1_048_576.0
    );
}
...
fn main() {
    ...
    count(
        backend
            .view::<HistoryIndicesTable<SlotAllocations>>()
            .unwrap(),
    );
    count(
        backend
            .view::<HistoryChangeTable<SlotAllocations>>()
            .unwrap(),
    );
}
